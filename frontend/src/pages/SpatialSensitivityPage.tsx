...
import { Copy, Download, Plus, Play, Save, Square, Trash2 } from 'lucide-react';
...
export function SpatialSensitivityPage({ active }: { active: boolean }) {
  ...
  return <Stack gap="md"><div><Title order={2}>Spatial ROI Sensitivity</Title><Text c="dimmed">Vollauflösende räumliche Ereignisanalyse mit einer festen, rein auswertenden ROI.</Text></div>
    ...
    <Paper withBorder p="md"><Stack gap="md"><MultiSelect label="Train/Test-Datensätze" data={datasets.map(d => ({ value: String(d.id), label: d.name, disabled: d.invalid_rule_count > 0 }))} value={datasetIds} onChange={value => { setDatasetIds(value); setEvents(current => current.filter(e => e.datasetId && value.includes(e.datasetId))); if (sourceDataset && !value.includes(sourceDataset)) setSourceDataset(null); }} searchable />
      ...
      <div><Title order={4}>Illustrative Rohbilder</Title><Text size="sm" c="dimmed">Diese zwei Bilder dienen nur der visuellen Einordnung und der Warp-Vorschau. Für jeden Zielzeitpunkt wird das nächstgelegene valide Bild innerhalb des zugehörigen Normal- beziehungsweise Ereignisintervalls geladen. Die eigentliche Analyse verwendet die oben konfigurierte reproduzierbare Zufallsstichprobe.</Text></div><SimpleGrid cols={{ base:1, md:3 }}><Select label="Beispielereignis" data={events.map(e => ({value:e.key,label:e.id}))} value={exampleEvent} onChange={setExampleEvent}/><DateTime24Input label="Normal-Zielzeitpunkt" value={normalExampleTime} onChange={setNormalExampleTime}/><DateTime24Input label="Ereignis-Zielzeitpunkt" value={eventExampleTime} onChange={setEventExampleTime}/></SimpleGrid><Button variant="light" onClick={loadExamples} disabled={!chosenEvent || !normalExampleTime || !eventExampleTime || busy}>Beispielbilder laden</Button>
    ...
    {displayedRun && <Paper withBorder p="md"><Group justify="space-between"><div><Text fw={600}>{displayedRun.current_step}</Text><Text size="sm" c="dimmed">{displayedRun.processed_images}/{displayedRun.total_images ?? '—'} Bilder · {displayedRun.failed_images} ungültig</Text></div><Group>{contentMatchesLoaded && loadedConfiguration?.latest_finished_run_id === displayedRun.id && <Badge color="green" variant="light">Gespeichertes Ergebnis</Badge>}<Badge color={displayedRun.status==='finished'?'green':displayedRun.status==='failed'?'red':displayedRun.status==='aborted'?'orange':'blue'}>{displayedRun.status}</Badge>{['queued','running'].includes(displayedRun.status)&&<Button color="red" variant="light" leftSection={<Square size={14}/>} loading={busy} onClick={abortRun}>Analyse abbrechen</Button>}</Group></Group>{['queued','running'].includes(displayedRun.status)&&<Progress value={Math.min(100, progress)} animated/>}{displayedRun.error_message&&<Alert mt="sm" color="red">{displayedRun.error_message}</Alert>}</Paper>}
