import { Accordion, Tabs, ThemeIcon, ActionIcon, Alert, Badge, Button, Group, Image, Loader, MultiSelect, NumberInput, Paper, Progress, Select, SimpleGrid, Stack, Text, TextInput, Title } from '@mantine/core';
import { mergeSpatialRunUpdate, spatialAbortState } from '../spatialSensitivity/abortState';
import { SpatialSensitivityResults } from '../spatialSensitivity/SpatialSensitivityResults';
import { SPATIAL_VERSION } from '../spatialSensitivity/metrics';
import { notifications } from '@mantine/notifications';
import { Copy, Plus, Play, Save, Square, Trash2, Scan, Settings2, Images, BarChart3 } from 'lucide-react';
import { PointerEvent, useEffect, useMemo, useRef, useState } from 'react';
import { abortSpatialSensitivityRun, createSpatialSensitivityConfiguration, createSpatialSensitivityRun, deleteSpatialSensitivityConfiguration, getSpatialSensitivityRun, listSpatialSensitivityConfigurations, listSpatialSensitivityRuns, listTrainingDatasets, previewSpatialSensitivity, previewSpatialSensitivityWarp, updateSpatialSensitivityConfiguration } from '../api';
...
const runLabels: Record<string, string> = { queued: 'Wartet auf Start', running: 'Wird berechnet', finished: 'Abgeschlossen', failed: 'Fehlgeschlagen', aborted: 'Abgebrochen', abort_requested: 'Abbruch angefordert …', indexing_candidates: 'Bildauswahl vorbereiten', loading_normal_sample: 'Normalbilder laden', loading_event_sample: 'Ereignisbilder laden', calculating_normal_statistics: 'Normalzustand auswerten', calculating_event_statistics: 'Ereignis auswerten', aggregating: 'Ergebnisse zusammenführen' };
...
export function SpatialSensitivityPage({ active }: { active: boolean }) {
  ...
  useEffect(() => { if (!active || !run || !['queued','running'].includes(run.status)) return; const timer = window.setInterval(() => getSpatialSensitivityRun(run.id).then(async next => { setRun(current => current ? mergeSpatialRunUpdate(current, next) : current); setRuns(current => current.map(item => item.id === next.id ? mergeSpatialRunUpdate(item, next) : item)); if (next.status === 'finished') setConfigurations(await listSpatialSensitivityConfigurations()); }).catch(e => setError(String(e))), 1500); return () => window.clearInterval(timer); }, [active, run?.id, run?.status]);
  ...
  const analysisConfig = useMemo<SpatialSensitivityAnalysisConfig | null>(() => {
    ...
    return {
      ...
      analysis_version: SPATIAL_VERSION, normal_window_hours: Number(hours), epsilon: Number(epsilon), roi_points: points,
  ...
  const valid = preview && datasetIds.length > 0 && events.length > 0 && events.every(e => e.id.trim() && e.datasetId && e.normalStart && e.start && e.end && new Date(e.normalStart) < new Date(e.start) && new Date(e.end) >= new Date(e.start)) && Number(hours) > 0 && Number(epsilon) > 0 && Number.isInteger(Number(normalSampleSize)) && Number(normalSampleSize) > 0 && Number(normalSampleSize) <= 1000 && Number.isInteger(Number(eventSampleSize)) && Number(eventSampleSize) > 0 && Number(eventSampleSize) <= 1000 && Number.isInteger(Number(samplingSeed)) && Number(samplingSeed) >= 0;
  ...
  async function abortRun() { if (!displayedRun || abortState.disabled || busy) return; setBusy(true); setError(null); try {
    ...
    notifications.show({ color: 'orange', title: 'Abbruch angefordert', message: next.status === 'aborted' ? `Lauf #${next.id} ist abgebrochen.` : next.abort_requested_at ? `Lauf #${next.id}: Abbruch angefordert, automatische harte Beendigung nach 10 Sekunden.` : `Lauf #${next.id} ist bereits beendet.` });
  } catch (e) { setError(e instanceof Error ? e.message : String(e)); } finally { setBusy(false); } }
  ...
  const abortState = spatialAbortState(displayedRun);
  return <Stack gap="lg" pb="xl">
    ...
    <Paper withBorder p="lg" radius="md"><Stack gap="md"><Group><ThemeIcon variant="light" radius="xl">1</ThemeIcon><div><Title order={4}>Datensätze & feste ROI</Title><Text size="sm" c="dimmed">Lade ein Normalbild und positioniere die vier Eckpunkte in Originalkoordinaten.</Text></div></Group><MultiSelect label="Train/Test-Datensätze" data={datasets.map(d => ({ value: String(d.id), label: d.name, disabled: d.invalid_rule_count > 0 }))} value={datasetIds} onChange={value => { setDatasetIds(value); setEvents(current => current.filter(e => e.datasetId && value.includes(e.datasetId))); if (sourceDataset && !value.includes(sourceDataset)) setSourceDataset(null); }} searchable />
      ...
      <SimpleGrid cols={{ base: 1, md: 2 }}><NumberInput label="Lokales Normalfenster (Stunden)" description="Vorgabe für den automatisch angelegten Normalstart neuer Ereignisse." min={0.01} value={hours} onChange={changeHours} /><NumberInput label="MAD-Epsilon (fest)" description="R = Änderung / (MAD + 1), ohne Skalierungsfaktor." readOnly value={epsilon} /></SimpleGrid>
      ...
      <SimpleGrid cols={{ base: 1, md: 3 }}><NumberInput label="Normalbilder" description="Maximale Zahl valider Bilder aus dem Normalzustand." min={1} max={1000} step={100} allowDecimal={false} value={normalSampleSize} onChange={setNormalSampleSize}/><NumberInput label="Ereignisbilder" description="Maximale Zahl valider Bilder aus dem Ereigniszeitraum." min={1} max={1000} step={100} allowDecimal={false} value={eventSampleSize} onChange={setEventSampleSize}/><NumberInput label="Zufalls-Seed" description="Gleicher Seed und gleiche Konfiguration ergeben dieselbe Auswahl." readOnly value={samplingSeed}/></SimpleGrid>
    ...
    {run && ['queued','running'].includes(run.status) && view !== 'results' && <Paper withBorder radius="md" p="md" style={{position:'sticky', bottom:16, zIndex:20, background:'var(--mantine-color-body)', boxShadow:'var(--mantine-shadow-md)'}}><Group justify="space-between"><Group><Loader size="sm"/><div><Text fw={600}>Lauf #{run.id} · {runLabels[run.status]}</Text><Text size="xs" c="dimmed">{abortState.message ?? runLabels[run.current_step] ?? run.current_step}</Text>{run.error_message && <Text size="xs" c="red">{run.error_message}</Text>}</div></Group><Group><Button variant="subtle" onClick={() => setView('results')}>Fortschritt ansehen</Button><Button color="red" variant="light" onClick={abortRun} disabled={busy || abortState.disabled} loading={busy} leftSection={<Square size={14}/>}>{abortState.buttonLabel}</Button></Group></Group></Paper>}
  </Stack>;
}
