...
import { Alert, Badge, Button, Code, Group, Modal, NumberInput, Paper, Progress, Select, Stack, Table, Text, TextInput, Title } from '@mantine/core';
import { sensorApi, timeSeriesApi } from '../api';
import { fieldDefaults, Flow, ModelFields } from '../timeSeries/ModelFields';
import type { ModelDefinition, Parameters, PipelinePreview, SensorModel, SensorPipeline, SensorRun, TimeSeriesDataset, TimeSeriesSplit } from '../timeSeries/types';
...
export function TimeSeriesPipelinesPage({ active }: { active: boolean }) {
  ...
  const [preview, setPreview] = useState<PipelinePreview>();
  ...
  useEffect(() => {
    if (!active || !split) { setPreview(undefined); return; }
    let cancelled = false;
    setPreview(undefined); setPreviewError('');
    const timer = setTimeout(() => sensorApi.preview({ split_id: Number(split), model_id: modelId ? Number(modelId) : undefined, window_length: lengthTouched.current ? length : undefined, training }).then(p => {
      if (!cancelled) { setPreview(p); if (!lengthTouched.current) setLength(p.window_length); }
    }).catch(e => !cancelled && setPreviewError(String(e))), 300);
    return () => { cancelled = true; clearTimeout(timer); };
  }, [active, split, modelId, length, training]);
  ...
  return <Stack p="md"><Group justify="space-between"><div><Title order={2}>Training Pipelines</Title><Text c="dimmed">Ein gemeinsamer Datenvertrag – vom gespeicherten Split bis zum ausgewählten Checkpoint.</Text></div><Button variant="light" onClick={() => { setId(undefined); setName('Neue Pipeline'); setSplit(null); setDataset(null); setModelId(null); setTraining({}); lengthTouched.current = false; setLength(36); }}>Neue Pipeline</Button></Group>
    ...
    <Paper withBorder radius="md" p="lg"><Stack><Title order={3}>{id ? 'Pipeline bearbeiten' : 'Pipeline konfigurieren'}</Title><TextInput label="Name" value={name} onChange={e => setName(e.currentTarget.value)} />
      ...
      <Group><Button loading={busy} disabled={!name.trim() || !model || !preview || !!preview.errors.length} onClick={() => save()}>Pipeline speichern</Button><Button loading={busy} variant="light" disabled={!name.trim() || !model || !preview || !!preview.errors.length} onClick={() => save(true)}>Speichern und Training starten</Button></Group>
    </Stack></Paper>
