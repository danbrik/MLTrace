...
import { Accordion, Alert, Badge, Button, Code, Group, Paper, SegmentedControl, Select, Stack, Table, Text, Title } from '@mantine/core';
...
export function TimeSeriesResultsPage({ active }: { active: boolean }) {
  ...
  useEffect(() => { if (!selected) return; let stale = false; setRun(undefined); setSensor('0'); setOffset(0); sensorApi.run(Number(selected)).then(r => !stale && setRun(r)).catch(e => !stale && setError(String(e))); return () => { stale = true; }; }, [selected]);
  useEffect(() => {
    if (!active || !run) return;
    let stale = false; setLoading(true); setSeries(undefined); setError('');
    sensorApi.series(run.id, subset, Number(sensor), space === 'scaled', offset).then(s => !stale && setSeries(s)).catch(e => !stale && setError(String(e))).finally(() => !stale && setLoading(false));
    return () => { stale = true; };
  }, [active, run, subset, sensor, space, offset]);
